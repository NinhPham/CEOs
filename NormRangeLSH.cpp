...
{
    cout << "Number of decimal values: " << PARAM_LSH_NUM_DECIMAL << endl;
    cout << "Number of partitions: " << PARAM_LSH_PARTITIONS << endl;
    cout << "Note: Number of hash tables must be divisible by number of partition" << endl;
    ...
    VECTOR_LSH_CODES = I64Vector(PARAM_DATA_N * PARAM_LSH_NUM_DECIMAL);
...
//        }
        ...
        for (l = 0; l < PARAM_LSH_NUM_DECIMAL; ++l)
        {
            iHashValue = 0;
            for (k = 64 * l; k < min(64 * (l + 1), PARAM_LSH_HASH_FUNCTIONS); ++k) // We use 64 bits
            {
                if (vecHash(k) > 0)
                   iHashValue += pow(2, k % 64);
            }

            VECTOR_LSH_CODES[iPointIdx * PARAM_LSH_NUM_DECIMAL + l] = iHashValue;
        }
    }
}
...
void build_RangeLSH_Table()
{
    ...
    for (l = 0; l < PARAM_LSH_HASH_TABLES; ++l)
        VECTOR_LSH_TABLES[l].reserve(PARAM_DATA_N);
...
void rangeLSH_Table_TopK()
{
    ...
    int q, l, k, n;
    int iIdx, iCountCandidates, iBucketSize;
...
//            cout << "There is an error! Norm of query is: " << vecQuery.norm() << endl;
        ...
        for (l = 0; l < PARAM_LSH_HASH_TABLES; ++l)
        {
            ...
            if (VECTOR_LSH_TABLES[l].find(iHashValue) != VECTOR_LSH_TABLES[l].end())
            {
                ...
                {
                    iIdx = vecBucket[n];
                    if (vecVisited[iIdx] == false)
                    {
                        vecTopB.push_back(iIdx);
                        vecVisited[iIdx] = true;

                        iCountCandidates++;

                        if (iCountCandidates >= PARAM_MIPS_DOT_PRODUCTS)
                        {
                            l = PARAM_LSH_HASH_TABLES + 1; //stop lookup
                            break;
                        }
                    }
                }
        ...
        dAvgCand += vecTopB.size();

        // In case there is no collsion
        if (vecTopB.size() == 0)
            continue;

        if (PARAM_TEST_SAVE_OUTPUT)
        {
            saveVector(vecTopB, "rangeLSH_Table_TopK_NoPost_" + int2str(q) + ".txt");
        }
        ...
        if (PARAM_TEST_SAVE_OUTPUT)
        {
            printf("Number of points in LSH %d \n", vecTopB.size());
...
void rangeLSH_Code_TopK()
{
    ...
    I64Vector vecI64Hash(PARAM_LSH_NUM_DECIMAL);
...
//            cout << "There is an error! Norm of query is: " << vecQuery.norm() << endl;
        ...
        for (n = 0; n < PARAM_DATA_N; ++n)
        {
            iHamDist = 0;
            for (l = 0; l < PARAM_LSH_NUM_DECIMAL; ++l)
                iHamDist += __builtin_popcountll(vecI64Hash[l] ^ VECTOR_LSH_CODES[n * PARAM_LSH_NUM_DECIMAL + l]); // ^ XOR
