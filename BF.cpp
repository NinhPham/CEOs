...
void BF_TopK_MIPS()
{
    ...
    // For each query
    #pragma omp parallel for reduction(+:distTime, topKTime)
    for (int q = 0; q < PARAM_QUERY_Q; ++q)
    {
        ...
        distTime += (float)durTime.count() *1e-3;
        ...
        {
            matTopK(k, q) = queTopK.top().m_iIndex;
            queTopK.pop();
        }
    }

    printf("Distance time in ms is %f \n", distTime);
    printf("Extract TopK time in ms is %f \n", topKTime);

    auto duration = chrono::duration_cast<chrono::milliseconds>(chrono::high_resolution_clock::now() - start);
    printf("BF Querying Time in ms is %f \n", (float)duration.count());
